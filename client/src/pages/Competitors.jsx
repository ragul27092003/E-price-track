...
const Competitors = () => {
  ...
  const urlList  = filtered.filter((c) => c.mappingType === 'URL');
  ...
  return (
    <Box sx={{ p: 3, bgcolor: '#ffffff', minHeight: '100vh' }}>
      <Stack direction="row" justifyContent="space-between" alignItems="center" mb={3}>
        <TextField
          placeholder="Search competitors..." size="small" value={searchTerm}
          onChange={(e) => setSearchTerm(e.target.value)} sx={{ width: 300, bgcolor: '#f9f9f9' }}
      ...
      <Dialog open={isModalOpen} onClose={() => setIsModalOpen(false)} maxWidth="md" fullWidth>
        ...
        <DialogContent sx={{ pt: 3 }}>
          <Grid container spacing={3} sx={{ mt: 1 }}>
            <Grid item xs={12} md={6}>
              ...
              <TextField fullWidth size="small" placeholder="Enter competitor name" value={newCompetitor.name}
                onChange={(e) => setNewCompetitor({ ...newCompetitor, name: e.target.value })} />
            </Grid>
            <Grid item xs={12} md={6}>
              <Typography variant="caption" color="text.secondary" fontWeight="bold">Competitor Website</Typography>
              <TextField fullWidth size="small" placeholder="https://example.com" value={newCompetitor.website}
                onChange={(e) => setNewCompetitor({ ...newCompetitor, website: e.target.value })} />
            </Grid>
            <Grid item xs={12} md={4}>
              <Typography variant="caption" color="text.secondary" fontWeight="bold">Search URL</Typography>
              <TextField fullWidth size="small" value={newCompetitor.searchUrl}
            ...
            <Grid item xs={12} md={4}>
              ...
              <TextField fullWidth size="small" type="color" value={newCompetitor.color}
                onChange={(e) => setNewCompetitor({ ...newCompetitor, color: e.target.value })} sx={{ p: 0 }} />
            </Grid>
            <Grid item xs={12} md={4}>
        ...
        <DialogActions sx={{ p: 3, borderTop: '1px solid #eee' }}>
          <Button onClick={() => setIsModalOpen(false)} sx={{ bgcolor: '#6c757d', color: '#fff', '&:hover': { bgcolor: '#5a6268' }, textTransform: 'none' }}>Close</Button>
          <Button onClick={handleSaveCompetitor} sx={{ bgcolor: '#28a745', color: '#fff', '&:hover': { bgcolor: '#218838' }, textTransform: 'none' }}>Save Competitor</Button>
        </DialogActions>
      </Dialog>
    </Box>
  );
