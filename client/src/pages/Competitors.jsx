...
const LogoCell = ({ data, isSuperAdmin, onLogoUploaded }) => {
  ...
  const handleFileChange = async (e) => {
    ...
    try {
      ...
      setLocalLogo(data.logo || "");
      alert(
        err?.response?.data?.message || "Logo upload failed. Please try again.",
      );
    } finally {
      setUploading(false);
      e.target.value = "";
  ...
  return (
    <div className="flex items-center gap-3 w-full md:w-[20%]">
      <div className="w-10 h-10 rounded border border-gray-200 flex items-center justify-center overflow-hidden shrink-0">
        {logoSrc ? (
          <img
            src={logoSrc}
  ...
  return (
    ...
    >
      ...
      <div className="w-full md:w-[17%] flex justify-between md:block">
        ...
        <div>
          {isSyncing ? (
            ...
            <span className="text-gray-400 dark:text-slate-500 font-bold text-sm">
              ○ Offline
            </span>
          )}
          {data.isRunning ? (
      ...
      <div className="w-full md:w-[13%] flex justify-between md:block">
        ...
        <span
          className={`text-sm font-bold ${isNegDelta ? "text-red-600" : "text-green-600"}`}
        >
      ...
      <div className="w-full md:w-[17%] flex justify-between items-center md:block">
        ...
        <Sparkline
          ...
          avgPriceDelta={data.avgPriceDelta}
        />
      </div>
      ...
      <div className="w-full md:w-[10%] flex justify-between md:justify-end items-center">
        ...
        >
          <span
            className={`inline-block h-4 w-4 transform rounded-full bg-white dark:bg-slate-800 transition-transform ${isActive ? "translate-x-6" : "translate-x-1"}`}
          />
        </button>
      </div>
  ...
  <div className="border border-gray-200 dark:border-slate-700 rounded-lg overflow-hidden mb-8 shadow-sm">
    ...
    <div className="bg-[#f8fafd] dark:bg-[#0f1624] p-4 md:p-5">
      <div className="hidden md:flex px-4 mb-3">
        <span className="text-[11px] font-bold text-gray-500 dark:text-slate-400 uppercase tracking-wider w-[20%]">
          Competitor
        </span>
        <span className="text-[11px] font-bold text-gray-500 dark:text-slate-400 uppercase tracking-wider w-[17%]">
          Status &amp; Last Sync
        </span>
        <span className="text-[11px] font-bold text-gray-500 dark:text-slate-400 uppercase tracking-wider w-[13%]">
          Avg. Price Delta
        </span>
        <span className="text-[11px] font-bold text-gray-500 dark:text-slate-400 uppercase tracking-wider w-[13%]">
          Products Tracked
        ...
        <span className="text-[11px] font-bold text-gray-500 dark:text-slate-400 uppercase tracking-wider w-[10%] text-right">
          Activation
        </span>
        {isSuperAdmin && (
          <span className="text-[11px] font-bold text-gray-500 dark:text-slate-400 uppercase tracking-wider w-[10%] text-right">
...
const Competitors = () => {
  ...
  const availableEanList = availableCompetitors.filter(
    (c) => c.mappingType === "EAN",
  );
  const availableNonEanList = availableCompetitors.filter(
    (c) => c.mappingType === "NON_EAN",
  );
          ...
          <div className="bg-white dark:bg-[#151a2a] rounded-lg w-full max-w-3xl overflow-hidden shadow-2xl">
            ...
            <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6 text-left">
              <div className="space-y-1">
                <label className="text-[11px] font-bold text-gray-500 dark:text-slate-400 uppercase">
                  Competitor Name
                </label>
                <input
            ...
            <div className="px-6 py-4 bg-slate-50 dark:bg-[#151a2a] flex justify-end gap-3 border-t dark:border-slate-700">
              <button
                ...
                className="px-5 py-2 bg-gray-500 text-white rounded text-sm"
              >
                Close
              </button>
              <button
