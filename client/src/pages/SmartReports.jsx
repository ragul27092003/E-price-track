...
import {
  fetchSmartReportProducts,
  fetchSmartReportProductDetail,
  exportSmartReportProducts,
...
function RankBadge({ rank, total }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[11px] font-bold ring-1 ${colors}`}
    >
      {label}
    </span>
  );
...
function MetricCard({ label, value, sub, accent }) {
  return (
    <div>
      ...
      <p
        className={`mt-1 text-xl font-black ${accent || "text-slate-800 dark:text-white"}`}
      >
        {value}
      </p>
      {sub && (
...
function HigherByBadge({ amount, label = "higher" }) {
  ...
  return (
    <div
      className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-[12px] font-bold ${isGood ? "bg-sky-100 text-sky-700" : "bg-rose-100 text-rose-700"}`}
    >
      <svg
        className={`w-3 h-3 ${isGood ? "" : "rotate-180"}`}
        viewBox="0 0 24 24"
        fill="none"
        stroke="currentColor"
        strokeWidth="3"
...
function getChronologicalHistory(priceHistory = [], days = 30) {
  ...
  return [...sliced].sort((a, b) => {
    ...
    return String(a?.display_date || "").localeCompare(
      String(b?.display_date || ""),
    );
  });
}
...
function HistoricalPriceChart({ product, days, competitorMeta }) {
  ...
  const slugs = useMemo(
    () => getCompPrices(product).map((c) => c.slug),
    [product],
  );
  ...
  const labels = [
    "My Price",
    ...slugs.map((s) => competitorMeta?.[s]?.name || s),
  ];
  ...
  const chartData = useMemo(
    () =>
      history.map((h) => {
        ...
        slugs.forEach((s) => {
          row[s] = parsePrice(h.competitors?.[s]);
        });
        return row;
      }),
    [history, slugs],
    ...
    <div className="flex flex-col h-full w-full">
      ...
      <div className="flex-1 relative w-full min-h-[180px]" ref={wrapperRef}>
        {hovered && (
          ...
          >
            <p className="font-bold text-slate-600 dark:text-slate-400 mb-1.5 border-b border-slate-100 dark:border-slate-700/50 pb-1">
              {hovered.date}
            </p>
            {dataKeys.map((k, i) => {
              const val = hovered[k];
...
function PriceGapChart({ product }) {
  ...
  const data = useMemo(() => {
    return getChronologicalHistory(product?.price_history_30days || [], 30)
      .map((h) => {
        ...
        const avg = compPrices.length
          ...
          : null;
        const gap =
          ourPrice !== null && avg !== null ? Math.round(ourPrice - avg) : null;
  ...
  const linePath = data
    .map((d, i) => `${i === 0 ? "M" : "L"}${getX(i)} ${getY(d.gap)}`)
    .join(" ");
  ...
  return (
    <div className="flex flex-col h-full w-full">
      <div
        className="flex-1 relative w-full h-full min-h-[130px]"
        ref={wrapperRef}
      >
        {hovered && (
          <div
          ...
          >
            ...
            <div className="flex items-center justify-between gap-3 mt-0.5">
              ...
              <span
                className={`font-bold ${hovered.gap < 0 ? "text-emerald-600" : hovered.gap > 0 ? "text-rose-600" : "text-slate-600"}`}
              >
                {hovered.gap > 0 ? "+" : ""}
                {fmt(hovered.gap)}
              </span>
        ...
        >
          ...
          <path
            ...
            clipPath="url(#pgc-above-zero)"
          />
          <path
            d={areaPath}
            fill="rgba(34,197,94,0.18)"
            clipPath="url(#pgc-below-zero)"
          />
          <path
            d={linePath}
            fill="none"
          ...
          {hoverIdx !== null && hovered && (
            <g>
              ...
              <circle
                ...
                cy={getY(hovered.gap)}
                r="4.5"
                fill={hovered.gap < 0 ? "#16a34a" : "#dc2626"}
                stroke="#fff"
                strokeWidth="2"
              />
            </g>
...
function formatCountValue(val) {
  const cleaned = parseMetaValue(val);
  if (cleaned === undefined) return undefined;
  if (typeof cleaned === "number") {
    if (cleaned > 0) return cleaned.toLocaleString("en-IN");
  } else {
    const rawStr = String(cleaned).trim();
    if (rawStr.length > 0) {
...
function CompetitorTable({ product, tab, competitorMeta }) {
  ...
  const allEntries = [
    {
      ...
      slug: "me",
      price: ourPrice,
      storePrice: ourStorePrice,
      sapPrice: ourSapPrice,
      isMe: true,
      rating: parseMetaValue(product.product_rating ?? product.rating),
      reviewCount: parseMetaValue(
        product.product_review ?? product.reviewCount ?? product.review_count,
      ),
    },
    ...listedComps.map((c) => ({
      ...c,
      ...
      sapPrice: null,
      isMe: false,
      rating: parseMetaValue(c.product_rating ?? c.rating),
      reviewCount: parseMetaValue(
        c.product_review ?? c.reviewCount ?? c.review_count,
      ),
    })),
  ].sort((a, b) => {
  ...
  const rank1Price =
    allEntries.find((entry) => entry.price !== null)?.price ?? null;
  const colLabel =
    tab === "Negative Trend"
      ? "Higher By"
      : tab === "Positive Trend"
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="bg-slate-50 dark:bg-[#151a2a] border-b border-slate-200 dark:border-slate-700">
            <th className="text-left px-4 py-2.5 font-semibold text-slate-600 dark:text-slate-400 text-xs uppercase tracking-wide">
              Competitor
            </th>
            <th className="text-center px-4 py-2.5 font-semibold text-slate-600 dark:text-slate-400 text-xs uppercase tracking-wide">
              Rating
            </th>
            <th className="text-center px-4 py-2.5 font-semibold text-slate-600 dark:text-slate-400 text-xs uppercase tracking-wide">
              Review
            </th>
            <th className="text-right px-4 py-2.5 font-semibold text-slate-600 dark:text-slate-400 text-xs uppercase tracking-wide">
              Web Price
            </th>
            <th className="text-center px-4 py-2.5 font-semibold text-slate-600 dark:text-slate-400 text-xs uppercase tracking-wide">
              Rank
        ...
        <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
          {allEntries.map((entry, i) => {
            ...
            let diffLabel = "";
            let diffColor = "";
            const formattedCount = formatCountValue(entry.reviewCount);
            ...
            return (
              <tr
                key={entry.slug}
                className={`transition-colors ${entry.isMe ? "bg-blue-50/60 dark:bg-blue-950/20 font-semibold" : "hover:bg-slate-50 dark:hover:bg-slate-800/50"}`}
              >
                  ...
                  <div className="flex items-center gap-3">
                    {entry.isMe ? (
                      ...
                      <>
                        ...
                        {rankNum === 1 && (
                          <span className="text-[10px] font-bold text-emerald-600 bg-emerald-50 dark:bg-emerald-950/30 dark:text-emerald-400 rounded-full px-1.5 py-0.5">
                            Rank 1
                          </span>
                        )}
                      </>
                ...
                <td className="px-4 py-3 text-right font-bold text-slate-800 dark:text-white">
                  {entry.price === null ? (
                    entry.url && !entry.isMe ? (
                      ...
                      <span className="text-rose-600 dark:text-rose-400 text-xs font-semibold uppercase tracking-wide">
                        Out of Stock
                      </span>
                    )
                  ) : (
                ...
                <td
                  className={`px-4 py-3 text-right text-xs font-semibold ${diffColor}`}
                >
                  {diffLabel}
                </td>
                <td className="px-4 py-3 text-center">
...
function HeaderMetrics({ product, tab, storePriceLabel = "Store Price" }) {
  ...
  if (tab === "Easy Gain") {
    ...
    extraContent = (
      <CheaperByBadge
        amount={lowestComp ? lowestComp.price - ourPrice : null}
      />
    );
    ...
    extraContent = (
      <CheaperByBadge
        amount={
          highestCompPrice && ourPrice !== null
            ? highestCompPrice - ourPrice
            : null
        }
      />
    );
    ...
    extraContent = (
      <HigherByBadge
        amount={
          lowestComp && ourPrice !== null ? lowestComp.price - ourPrice : null
        }
        label="higher"
      />
    );
  } else if (tab === "Neutral Trend") {
    extraLabel = "Price Status";
    extraContent = (
      <span className="inline-flex items-center rounded-full bg-blue-100 px-3 py-1 text-[12px] font-bold text-blue-700">
        Tied Price
      </span>
    );
  } else if (tab === "Negative Trend") {
    ...
    extraContent = (
      <HigherByBadge
        amount={
          lowestComp && ourPrice !== null ? ourPrice - lowestComp.price : null
        }
        label="above rank 1"
      />
    );
  ...
  return (
    <button
      ...
      className={`flex w-full items-center gap-3 rounded-lg px-3 py-3 text-left transition-all ${
        isSelected
          ? "bg-[#ebf5fb] dark:bg-blue-950/40 ring-1 ring-[#2B86C5]/30"
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <p
          className={`font-semibold text-[13px] truncate ${isSelected ? "text-[#1e6191] dark:text-blue-400" : "text-slate-800 dark:text-white"}`}
        >
        ...
        <p className="text-[12px] font-bold text-slate-700 dark:text-slate-300 mt-0.5">
          {fmt(ourPrice)}
        </p>
      </div>
...
function exportSmartReportCSV(products, tab, competitorMeta, exportType = "A") {
  ...
  const rows = products.map((p) => {
    ...
    const lowest = comps.length
      ? comps.reduce((min, c) => (c.price < min.price ? c : min))
      : null;
    const lowestMeta = lowest ? competitorMeta?.[lowest.slug] || {} : {};
    const lowestName = lowest
      ? lowestMeta.name || lowest.name || lowest.slug
      : "";
...
export default function SmartReports() {
  ...
  const { competitors, setCompetitors, activeStoreId, fetchMerchant } =
    useStore();
  ...
  const storePriceLabel = String(activeStoreId || "")
    .toLowerCase()
    .includes("nandilathgmart")
    ? "MRP Price"
    : "Store Price";

  const [activeTab, setActiveTab] = useState(
    location.state?.tab || "Easy Gain",
  );
  ...
  useEffect(() => {
    ...
    fetchSmartReportProductDetail(selectedEan)
      ...
      .catch((err) => {
        ...
        if (!cancelled) {
          const fallback =
            listProducts.find((p) => p.product_ean_id === selectedEan) || null;
  ...
  const selectedProduct = useMemo(() => {
    ...
    return (
      listProducts.find((p) => p.product_ean_id === selectedEan) ||
      listProducts[0]
    );
  ...
  const handleExport = async () => {
    setExporting(true);
    try {
      const result = await exportSmartReportProducts({
        tab: activeTab,
        search,
      });
      exportSmartReportCSV(
        result.data || [],
        activeTab,
        competitorMeta,
        exportType,
        ...
        <div className="animate-in fade-in slide-in-from-bottom-2 duration-500">
            ...
            {TABS.map((tab) => (
              <button
                ...
                className={`pb-3 text-sm font-semibold transition-all relative flex shrink-0 items-center gap-2 mr-5 whitespace-nowrap ${
                  activeTab === tab
                    ? "text-[#1e6191] dark:text-blue-400"
                    : "text-slate-400 hover:text-slate-600"
                }`}
              >
                {tab}
                <span
                  className={`text-[10px] rounded-full px-1.5 py-0.5 font-bold ${activeTab === tab ? "bg-[#dbeafe] text-[#1e6191]" : "bg-slate-100 text-slate-500"}`}
                >
                  {tabCounts[tab]}
                </span>
                {activeTab === tab && (
          ...
          {loading && listProducts.length === 0 ? (
            <div className="flex items-center justify-center py-24 text-slate-400">
              <svg
                className="animate-spin h-6 w-6 mr-3 text-[#2B86C5]"
                viewBox="0 0 24 24"
                fill="none"
              >
                <circle
              ...
              >
                ...
                {listProducts.map((p) => (
                  <SidebarProduct
                    ...
                    product={p}
                    isSelected={
                      selectedProduct?.product_ean_id === p.product_ean_id
                    }
                ...
                {loadingMore && (
                  <div className="flex items-center justify-center py-4 text-slate-400">
                    <svg
                      className="animate-spin h-5 w-5 mr-2 text-[#2B86C5]"
                      viewBox="0 0 24 24"
                      fill="none"
                    >
                      <circle
                ...
                <div className="space-y-5">
                  {detailLoading && (
                    <div className="flex items-center gap-2 text-sm text-slate-400">
                      <svg
                        className="animate-spin h-4 w-4 text-[#2B86C5]"
                        viewBox="0 0 24 24"
                        fill="none"
                      >
                        <circle
                  ...
                  {activeTab !== "Non Competitors" &&
                    getListedCompetitors(selectedProduct).length > 0 && (
