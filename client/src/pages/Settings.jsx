...
import {
  ...
  X, Check, AlertCircle, Users, FileText, ChevronDown,
} from "lucide-react";
...
function Avatar({ size = "lg", primaryColor }) {
  const sizeClass = size === "lg" ? "h-[72px] w-[72px] text-sm" : "h-9 w-9 text-xs";
  return (
    <div className={`${sizeClass} rounded-full flex flex-col items-center justify-center text-white font-bold shadow-sm tracking-widest leading-none`}
...
// ── MyAccountTab ────────────────────────────────────────────────────────────
function MyAccountTab({ primaryColor, setPrimaryColor, secondaryColor, setSecondaryColor }) {
  ...
  useEffect(() => {
    ...
    if (profile) {
      ...
      setUserType(profile.userType   || "store_admin");
    ...
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-6 max-w-full pb-10 pt-2">
      ...
      <div className="bg-gray-50 border border-gray-200 rounded-xl p-6">
        ...
        <div className="flex flex-wrap items-center gap-6">
          <Button variant="outline" className="text-gray-700 font-semibold border-gray-300 bg-white hover:bg-gray-50">Upload Logo</Button>
          <div className="flex flex-col items-start leading-none ml-2">
            ...
            <span className="text-[9px] font-bold tracking-wide mt-0.5" style={{ color: primaryColor }}>ELECTRONICS</span>
          </div>
          <div className="flex flex-wrap items-center gap-6 ml-auto">
            {[["Primary Brand Color", primaryColor, setPrimaryColor], ["Secondary Brand Color", secondaryColor, setSecondaryColor]].map(([label, value, setter]) => (
              <div key={label}>
                ...
                <div className="relative flex items-center gap-2 border border-gray-300 bg-white rounded-md px-3 py-1.5 cursor-pointer hover:bg-gray-100">
                  ...
                  <div className="w-5 h-5 rounded-sm" style={{ backgroundColor: value }} />
      ...
      <div className="bg-gray-50 border border-gray-200 rounded-xl p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
          ...
          <div>
            ...
            <Input value={companyName || userName} readOnly className="border-gray-300 cursor-default bg-gray-50 text-gray-700" />
          </div>
          ...
          <div>
            ...
            <div className="relative">
              <select value={country} onChange={(e) => setCountry(e.target.value)} className="w-full h-10 rounded-md border border-gray-300 bg-white px-3 text-sm text-gray-900 outline-none appearance-none">
          ...
          <div>
            ...
            <div className="relative">
              <Input type={showNew ? "text" : "password"} value={newPassword} onChange={(e) => setNewPassword(e.target.value)}
                placeholder="New Password" className="border-gray-300 pr-10 bg-white" />
            ...
            <div className="mt-3">
              <div className="flex gap-1.5">
                {[1,2,3,4].map((i) => (
                  <div key={i} className={`h-1.5 flex-1 rounded-full ${i <= score ? (score === 1 ? "bg-red-500" : score === 2 ? "bg-orange-500" : "bg-green-500") : "bg-gray-200"}`} />
                ))}
              </div>
              <div className={`text-[10px] font-bold mt-1.5 ${score === 1 ? "text-red-500" : score === 2 ? "text-orange-500" : score >= 3 ? "text-green-600" : "text-gray-500"}`}>
                {text || "Strength"}
              </div>
            </div>
          </div>
