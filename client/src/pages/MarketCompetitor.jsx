...
function CompetitorLogo({ name = "", slug = "", logo = "", size = "md" }) {
  ...
  if (logoSrc && !imgErr) {
    return (
      <div
        className={`flex ${dim} items-center justify-center rounded overflow-hidden border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 shrink-0`}
      >
        <img
          src={logoSrc}
          alt={name}
  ...
  return (
    <div
      className={`flex ${dim} min-w-[26px] items-center justify-center rounded px-1 text-[8px] font-bold uppercase tracking-wider text-white shrink-0`}
    ...
    <div className="flex flex-col items-start gap-2">
      {hasData && !isZero && (
        ...
        >
          ...
          />
          <span>
            {Math.abs(numericValue)}% {isNeg ? "below" : "above"} market
          </span>
        </div>
      )}
      {ean && (
  ...
  return (
    <div className="flex items-center gap-8">
      ...
      <div className="relative">
        ...
        <br />
        <span className="font-bold text-slate-800 dark:text-white">
          {avg !== null ? `Avg: ₹${avg.toLocaleString("en-IN")}` : "—"}
        </span>
      </div>
      <div>
...
function MarketGapCell({ product, competitorMeta }) {
  ...
  const compName =
    competitorMeta?.[lowest.slug]?.name || lowest.name || lowest.slug;
  ...
  if (gap === 0)
    return (
      <div className="flex flex-col gap-1">
        <span className="inline-flex text-[11px] font-bold text-slate-500 bg-slate-100 rounded-full px-2.5 py-1 w-fit">
          Same price
        </span>
  ...
  if (gap > 0)
    return (
      <div className="flex flex-col gap-1">
        <span className="inline-flex text-[11px] font-bold text-rose-600 bg-rose-50 rounded-full px-2.5 py-1 w-fit">
          ↑ {fmtAmt(gap)} higher
        </span>
        <span className="text-[10px] text-slate-400 mt-0.5">
  ...
  return (
    <div className="flex flex-col gap-1">
      <span className="inline-flex text-[11px] font-bold text-emerald-600 bg-emerald-50 rounded-full px-2.5 py-1 w-fit">
        ↓ {fmtAmt(-gap)} cheaper
      </span>
      <span className="text-[10px] text-slate-400 mt-0.5">
        {fmtAmt(lowest.price)} ({compName}) vs {fmtAmt(ourPrice)} mine
      ...
      {listed.map((c) => {
        ...
        return (
          <div key={c.slug} className="flex flex-col gap-3">
            {/* First Row: Competitor Logo + Status Badge / Price + History Button */}
                ...
                c.url ? (
                  ...
                  >
                    <CompetitorLogo
                      ...
                      logo={meta.logo || ""}
                    />
                    <span className="font-bold text-rose-600 dark:text-rose-400 text-[10px] uppercase bg-rose-50 dark:bg-rose-950/30 px-2 py-0.5 rounded-md border border-rose-200 dark:border-rose-800">
                      Out of Stock
                    </span>
                  </a>
                ) : (
            ...
            {/* Second Row: Product Rating and Review Count directly below */}
            <div className="flex items-center gap-1.5">
              <CompetitorMeta
                rating={c.product_rating}
                reviewCount={c.product_review}
              />
            </div>
          </div>
        );
    ...
    <div className="flex items-center gap-4">
      ...
      <div>
        {linkUrl ? (
          ...
          <p
            className="font-bold text-slate-800 dark:text-white text-[13px] line-clamp-2 max-w-[220px]"
            title={product.product_name || "Unnamed Product"}
          >
            {product.product_name || "Unnamed Product"}
          </p>
...
function Pagination({
  ...
  const pageBtn = (p) => (
    <button
      ...
      className={`flex h-8 w-8 items-center justify-center rounded-md text-xs font-semibold transition-colors ${p === currentPage ? "bg-[#2B86C5] text-white border border-[#2B86C5]" : "border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-[#151a2a]"}`}
    >
      {p}
    </button>
  );
  ...
  return (
    <div className="flex items-center justify-between border-t border-slate-100 dark:border-slate-700/50 bg-white dark:bg-[#0b101e] px-5 py-3">
      <p className="text-[11px] font-medium text-slate-500 lowercase tracking-wide">
        {displayString}
      </p>
      <div className="flex items-center gap-1">
        <button
          ...
          className="flex h-8 w-8 items-center justify-center rounded-md border border-slate-200 dark:border-slate-700 text-slate-600 disabled:opacity-40 transition-colors"
        >
          ‹
        </button>
        {adjustedStart > 1 && (
...
function FilterSelect({ label, options, value, onChange }) {
  ...
  return (
    <div className="relative flex-1 min-w-[140px] sm:min-w-[160px] max-w-full sm:max-w-[220px]">
      <p className="mb-1.5 text-[11px] font-bold text-slate-500 uppercase tracking-wide">
        {label}
      </p>
      <div
...
function CompetitorTab({ data, isActive, onClick, liveCount }) {
  ...
  return (
    ...
    >
      ...
      {productCount !== null && (
        <span
          className={`rounded-full px-1.5 py-0.5 text-[10px] font-bold leading-none ${isActive ? "bg-white/20 text-white" : "bg-slate-100 dark:bg-slate-700 text-slate-500 dark:text-slate-300"}`}
        >
          {productCount === "loading"
            ? "…"
            : Number(productCount).toLocaleString("en-IN")}
        </span>
      )}
      {data.isRunning && (
        <span
      ...
      {isOffline ? (
        <span className="rounded bg-gray-200 px-1.5 py-0.5 text-[9px] font-black uppercase text-gray-500">
          Offline
        </span>
      ) : (
        <span
          className={`text-[10px] font-bold ${isActive ? "text-white/90" : isNegDelta ? "text-red-500" : "text-green-600"}`}
        >
...
const MarketCompetitor = () => {
  ...
  const eanList = competitors
    .filter((c) => c.mappingType === "EAN")
    .sort((a, b) => (a.name || "").localeCompare(b.name || ""));
  const nonEanList = competitors
    .filter((c) => c.mappingType === "NON_EAN")
    .sort((a, b) => (a.name || "").localeCompare(b.name || ""));
      ...
      <div className="p-3 sm:p-6 bg-white dark:bg-[#0b101e] min-h-screen font-sans">
              ...
              <div className="flex flex-wrap gap-2">
                {eanList.map((item) => (
                  <CompetitorTab
                    ...
                    liveCount={
                      item.slug === selectedCompetitor?.slug
                        ? productsLoading
                          ? "loading"
                          : totalItems
                        : undefined
        ...
        {!selectedCompetitor ? (
          <div className="py-16 text-center text-sm text-slate-400 italic">
            Select a competitor tab to view their products.
          </div>
        ) : (
          <div className="mt-5">
            ...
            <div className="mb-4 flex items-center gap-2.5">
              <CompetitorLogo
                ...
                slug={selectedCompetitor.slug}
                logo={selectedCompetitor.logo || ""}
              />
              ...
              <span className="ml-1 rounded-full bg-slate-100 dark:bg-slate-700 px-2 py-0.5 text-[11px] font-bold text-slate-500 dark:text-slate-300">
                {productsLoading
                  ? "…"
                  : `${totalItems.toLocaleString("en-IN")} ${totalItems === 1 ? "product" : "products"}`}
              </span>
            </div>
            ...
            <div className="mb-6 space-y-4">
                ...
                <div className="flex items-center gap-3 w-full sm:w-auto justify-end">
                  {canExport && (
                    <button
                      ...
                      disabled={exporting}
                      className="flex items-center gap-2 rounded-lg bg-[#2B86C5] px-4 py-2.5 text-sm font-medium text-white shadow-sm hover:bg-[#226fa3] disabled:opacity-60 disabled:cursor-not-allowed transition-colors"
                    >
                      {exporting ? (
                        <svg
                          viewBox="0 0 24 24"
              ...
              <div className="flex flex-wrap gap-3">
                <FilterSelect
                  label="Item Group"
                  options={productsMeta?.itemGroups || []}
                  value={itemGroupFilter}
                  onChange={(v) => {
                    setItemGroupFilter(v);
                    setCurrentPage(1);
                  }}
                />
                <FilterSelect
                  label="Brand"
                  options={productsMeta?.brands || []}
                  value={brandFilter}
                  onChange={(v) => {
                    setBrandFilter(v);
                    setCurrentPage(1);
                ...
                <FilterSelect
                  label="Category"
                  options={productsMeta?.categories || []}
                  value={catFilter}
                  onChange={(v) => {
                    setCatFilter(v);
                    setCurrentPage(1);
                      ...
                      <tbody className="divide-y divide-slate-100 dark:divide-slate-700/50">
                        {competitorProducts.length === 0 ? (
                          <tr>
                            ...
                            >
                              No products found for this competitor.
                            </td>
                          </tr>
                        ) : (
