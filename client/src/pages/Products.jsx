...
function parsePrice(raw) {
  if (raw === null || raw === undefined || raw === "No Result" || raw === "")
    return null;
  const n =
    typeof raw === "number"
      ? raw
...
function marketStats(product) {
  const prices = (product.price_history_30days || [])
    .map((h) =>
      typeof h.product_price === "number"
        ? h.product_price
        : parseFloat(h.product_price),
    )
...
function RankBadge({ product }) {
  ...
  let colorClass =
    "bg-slate-100 text-slate-700 dark:text-slate-300 dark:text-slate-600";
  ...
  return (
    <span
      className={`inline-flex items-center justify-center rounded-md px-2 py-1 text-xs font-bold ${colorClass}`}
    >
      {displayRank}
    </span>
  );
...
function PriceGapBadge({ value, ean }) {
  ...
  return (
    <div className="flex flex-col items-start gap-2">
      ...
      {hasData && !isZero && (
        <div
          className={`relative inline-flex items-center gap-1 rounded-full pr-3 pl-2 py-1 text-[11px] font-bold ${baseColors}`}
        >
          <svg
          ...
          <div
            className={`w-0.5 h-3 opacity-30 ${barColor} mx-0.5 rounded-full`}
          />
          <span>
  ...
  return (
    <div className="flex items-center gap-8">
      ...
      <div className="relative">
        ...
        <br />
        <span className="font-bold text-slate-800 dark:text-white">
          {avg !== null ? `₹${avg.toLocaleString("en-IN")}` : "—"}
        </span>
        {avg !== null && (
          <>
  ...
  if (gap === 0) {
    return (
      <div className="flex flex-col gap-1">
        <span className="inline-flex items-center text-[11px] font-bold text-slate-500 dark:text-slate-400 bg-slate-100 rounded-full px-2.5 py-1 w-fit">
          Same price
        </span>
            ...
            <div className="flex items-center gap-2 min-w-0 shrink">
              {isOos ? (
                c.url ? (
                  <a
                    ...
                    className="flex items-center gap-2 hover:opacity-75 transition-opacity cursor-pointer min-w-0"
                  >
                    <CompetitorLogo
                      name={c.name}
                      slug={c.slug}
                      logo={meta.logo || ""}
                    />
                    <span className="font-bold text-rose-600 dark:text-rose-400 text-[10px] uppercase tracking-wider bg-rose-50 dark:bg-rose-950/30 px-2 py-0.5 rounded-md border border-rose-200 dark:border-rose-800 whitespace-nowrap shrink-0">
                      Out of Stock
                    </span>
                  </a>
                ) : (
                ...
                >
                  ...
                  />
                  {hasPrice && (
                    <span className="font-bold text-slate-800 dark:text-white text-[13px] whitespace-nowrap">
              ...
              {!isOos && (
                <button
                  ...
                  className="group flex items-center justify-center h-6 w-6 rounded-full border border-sky-200 bg-sky-50 text-sky-600 hover:bg-sky-500 hover:text-white hover:border-sky-500 hover:shadow-sm transition-all shrink-0 ml-0.5"
...
function FilterSelect({ label, options, value, onChange }) {
  ...
  return (
    <div className="relative flex-1 min-w-[140px] sm:min-w-[160px] max-w-full sm:max-w-[220px]">
      <p className="mb-1.5 text-[11px] font-bold text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase tracking-wide">
        {label}
      </p>
      <div
                ...
                <button
                  ...
                  className={`w-full text-left px-3 py-2 text-sm transition-colors ${
                    i === cursor
                      ...
                      : opt === value
                        ? "bg-slate-50 dark:bg-[#151a2a] font-medium text-slate-800 dark:text-white"
    ...
    <div className="flex items-center justify-between border-t border-slate-100 dark:border-slate-700/50 bg-white dark:bg-[#0b101e] px-5 py-3">
      ...
      <div className="flex items-center gap-1">
        ...
        {adjustedStart > 1 && (
          <>
            {pageBtn(1)}
            <span className="px-1 text-slate-400 dark:text-slate-500 text-xs">
              …
            </span>
          </>
        )}
...
function ErrorState({ message, onRetry }) {
  return (
    <div className="flex min-h-[400px] flex-col items-center justify-center gap-3 text-center">
      ...
      <button
        onClick={onRetry}
        className="rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-4 py-2 text-sm font-medium text-slate-700 dark:text-slate-300 shadow-sm hover:bg-slate-50 dark:hover:bg-[#151a2a]"
      >
        Retry
      </button>
    </div>
    ...
    <div className="flex items-center gap-4">
      ...
      <div>
        {product.product_url ? (
          ...
          <p className="font-bold text-slate-800 dark:text-white text-[13px]">
            {product.product_name || "Unnamed Product"}
          </p>
        )}
...
function PriceCell({ product }) {
  ...
  const canPriceChange = useStore(
    (s) => (s.user?.webprice_access ?? "yes") === "yes",
  );
  ...
  return (
    <div className="flex flex-col gap-1.5 min-w-[110px]">
      <div className="flex items-center justify-between gap-3">
        ...
        <span className={`text-[12px] font-bold ${webPriceColorClass(rank)}`}>
          {fmt(web)}
        </span>
      </div>
      <div className="flex items-center justify-between gap-3">
    ...
    products.forEach((p) => {
      (p.competitor_prices || [])
        ...
        .forEach((c) => {
          if (!slugMap[c.slug])
            slugMap[c.slug] =
              competitorMeta?.[c.slug]?.name || c.name || c.slug;
        });
    });
...
function CompetitorHistoryModal({
  product,
  competitor,
  onClose,
  initialRange = 30,
  ...
  const compValues = history.map((h) =>
    parsePrice(h.competitors?.[competitor.slug]),
  );
      ...
      <div className="w-full max-w-2xl rounded-2xl bg-white dark:bg-[#151a2a] shadow-2xl border border-slate-200 dark:border-slate-700/60 overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 dark:border-slate-700/50 bg-slate-50 dark:bg-[#0b101e]">
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <button
              onClick={onClose}
              className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-slate-400 hover:bg-slate-200 hover:text-slate-600 transition-colors"
            >
              <svg
                viewBox="0 0 24 24"
                width="16"
        ...
        <div className="px-6 py-5">
          <div className="mb-3 flex items-center gap-4">
            ...
            <div className="flex items-center gap-1.5">
              <span
                className="inline-block h-2.5 w-2.5 rounded-[2px]"
                style={{ backgroundColor: competitor.color || "#94a3b8" }}
              />
              <span className="text-[11px] font-semibold text-slate-600 dark:text-slate-400">
                {competitor.name}
              </span>
            ...
            >
              ...
              >
                {yTicks.map((tick) => (
                  <g key={tick}>
                    ...
                    <text
                      ...
                      fontSize="10"
                      fill="#64748b"
                      className="dark:fill-gray-500 font-semibold"
                    >
                      {formatPrice(tick)}
                    </text>
                  </g>
                ...
                {history.map((h, i) => (
                  <text
                    key={i}
                    transform={`translate(${getX(i)},${pad.top + plotH + 14}) rotate(60)`}
                    textAnchor="start"
                    fontSize="7"
                    fill="#64748b"
                    className="dark:fill-gray-400 font-semibold"
                  >
                    {h.display_date || ""}
                  </text>
                ))}
                  ...
                  return (
                    <g key={s.name}>
                      <path
                        ...
                        fill="none"
                        stroke={s.color}
                        strokeWidth={s.isMain ? 3 : 2.5}
                        strokeLinecap="round"
                        strokeLinejoin="round"
                        opacity={s.isMain ? 1 : 0.85}
                      />
              ...
              {hoverIdx !== null && (
                <div
                  ...
                  style={{
                    left:
                      hoverIdx > history.length / 2
                        ? "auto"
                        : `${(getX(hoverIdx) / svgW) * 100}%`,
                    right:
                      hoverIdx > history.length / 2
                        ? `${100 - (getX(hoverIdx) / svgW) * 100}%`
                        : "auto",
                    top: "0px",
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center gap-1.5">
                            ...
                            />
                            <span
                              className={`text-[10px] ${s.isMain ? "font-bold text-slate-900 dark:text-white" : "font-medium text-slate-600 dark:text-slate-400"}`}
                            >
                              {s.name}
                            </span>
                          </div>
                          <span
                            className={`text-[10px] ${s.isMain ? "font-black text-blue-600 dark:text-blue-400" : "font-bold text-slate-800 dark:text-slate-200"}`}
                          >
                            {formatPrice(val)}
                          </span>
                        </div>
        ...
        <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-slate-100 dark:border-slate-700/50 bg-slate-50 dark:bg-[#151a2a]">
          <button
            onClick={onClose}
            className="rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-4 py-2 text-sm font-medium text-slate-600 dark:text-slate-400 hover:bg-slate-50 transition-colors"
          >
            Close
          </button>
        </div>
...
function ConfigureModal({ product, currentUserId, onClose, onSaved }) {
  ...
  const [groupName, setGroupName] = useState(
    product?.group_name || groupNameDefault,
  );
  ...
  const handleSave = async () => {
    ...
    setError("");
    try {
      const existingIds = Array.isArray(product?.user_alert_id)
        ? product.user_alert_id
        : [];
      const userAlertId =
        currentUserId && !existingIds.includes(currentUserId)
          ...
          : existingIds.length > 0
            ...
            : currentUserId
              ? [currentUserId]
              : [];
      await configureProduct(product._id, {
          ...
          <div>
            ...
            {product?.product_category && (
              <p className="mt-1 text-[11px] text-slate-400 dark:text-slate-500">
                From category:{" "}
                <span className="font-medium">{product.product_category}</span>
              </p>
            )}
          </div>
      ...
      <div className="w-full max-w-sm rounded-2xl bg-white dark:bg-[#151a2a] shadow-2xl border border-slate-200 dark:border-slate-700/60 overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 dark:border-slate-700/50 bg-slate-50 dark:bg-[#0b101e]">
          <div>
            ...
            <h2 className="font-bold text-slate-800 dark:text-white text-sm mt-0.5">
              Configure All Selected Products
            </h2>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="px-6 py-5">
          <p className="text-sm text-slate-600 dark:text-slate-300">
            ...
            selected products to Individual Item Group?
          </p>
          <p className="mt-2 text-xs text-slate-400 dark:text-slate-500">
            Each product will be assigned to its own group based on its
            category.
          </p>
        </div>
        <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-slate-100 dark:border-slate-700/50 bg-slate-50 dark:bg-[#151a2a]">
        ...
        <div className="px-6 py-5">
          {isBulk ? (
            <p className="text-sm text-slate-600 dark:text-slate-300">
              ...
              products from their Item Groups? This will clear their group name
              and alert settings.
            </p>
          ) : (
            <p className="text-sm text-slate-600 dark:text-slate-300">
              ...
              from its Item Group? This will clear its group name and alert
              settings.
            </p>
          )}
        </div>
...
function SortIcon({ active, direction }) {
  ...
  return (
    <svg
      className="shrink-0 text-[#2B86C5]"
      width="12"
      height="12"
      viewBox="0 0 24 24"
      fill="none"
...
const HandleWebPriceChange = async (product) => {
  ...
  if (newPrice !== undefined && newPrice !== false) {
    ...
    try {
      const response = await fetch(wepPriceApiUrl, {
        method: "POST",
        headers: {
...
export default function Products() {
  ...
  const onlineSlugs = new Set(
    competitors.filter((c) => c.isActive).map((c) => c.slug),
  );
  ...
  const setItemGroupFilter = (v) => {
    _setItemGroupFilter(v);
    setCurrentPage(1);
  };
  ...
  const load = async (page, searchQ, brand, cat, rank, itemGroup) => {
    ...
    try {
      ...
      const totalPages = Array.isArray(res)
        ? Math.ceil(res.length / ITEMS_PER_PAGE)
        : res?.totalPages || 1;
  ...
  const isConfigured = (p) =>
    !!(
      alertUserId &&
      Array.isArray(p.user_alert_id) &&
      p.user_alert_id.includes(alertUserId)
    );
  ...
  const sorted = [...filtered].sort((a, b) => {
    ...
    if (sortConfig.column === "name") {
      const cmp = (a.product_name || "")
        .toLowerCase()
        .localeCompare((b.product_name || "").toLowerCase());
      return sortConfig.direction === "asc" ? cmp : -cmp;
    }
    if (sortConfig.column === "price") {
      const aP = parsePrice(a.product_price),
        bP = parsePrice(b.product_price);
    ...
    if (sortConfig.column === "rank") {
      const aR = parseInt(
        a.user_notification_data?.rank_pos ?? a.rank_by ?? 9999,
        10,
      );
      const bR = parseInt(
        b.user_notification_data?.rank_pos ?? b.rank_by ?? 9999,
        10,
      );
  ...
  const handleConfigSaved = (productId, groupName, userAlertId) => {
    setProducts(
      products.map((p) =>
        p._id === productId
          ? { ...p, group_name: groupName, user_alert_id: userAlertId }
          : p,
      ),
    );
  };
  ...
  const handleRemove = async () => {
    ...
    try {
      if (removeTarget === "bulk") {
        ...
        await Promise.all(
          filtered.map((p) => removeProductConfiguration(p._id)),
        );
        setProducts(
          products.map((p) =>
            filteredIds.has(p._id)
              ? { ...p, group_name: "", user_alert_id: [] }
              : p,
          ),
        );
      } else {
  ...
  const handleBulkSave = async () => {
    ...
    try {
      ...
      await Promise.all(
        filtered.map((p) => {
          const groupName = p.product_category
            ? p.product_category.split(">")[0].trim()
            : p.group_name || "";
          const existingIds = Array.isArray(p.user_alert_id)
            ? p.user_alert_id
            : [];
          const userAlertId =
            currentUserId && !existingIds.includes(currentUserId)
    ...
    <>
          ...
          {competitorSlug && (
            <div className="flex items-center gap-3 rounded-xl border border-blue-200 bg-blue-50 px-4 py-3">
              ...
              <p className="text-sm font-medium text-blue-800 flex-1">
                Showing products sold by{" "}
                <strong>{competitorName || competitorSlug}</strong>
              </p>
              <button
                      ...
                      <tbody className="divide-y divide-slate-100">
                        {filtered.length === 0 ? (
                          ...
                          paginated.map((p) => (
                            <tr
                              key={p._id}
                              className="hover:bg-slate-50 dark:hover:bg-[#151a2a]/80 transition-colors"
                            >
                      ...
                      <tbody className="divide-y divide-slate-100">
                        {filtered.length === 0 ? (
                          ...
                          paginated.map((p) => {
                            ...
                            const active = (p.competitor_prices || []).filter(
                              (c) => c.price != null && c.price > 0,
                            );
                            return (
                              <tr
                                key={p._id}
                          ...
                          <tr className="border-b border-slate-100 bg-slate-50/50">
                            <th className="w-12 px-5 py-4">
                              <input
                                ...
                                onChange={() =>
                                  allConfigured
                                    ? setRemoveTarget("bulk")
                                    : setShowBulkConfirm(true)
                                }
                                title={
                                  allConfigured
                                    ? "Uncheck all — remove from Item Groups"
                                    : "Check all — configure Item Groups"
                                }
                              />
                            </th>
                            <th className="px-5 py-4 text-left text-xs font-semibold text-slate-500">
                            ...
                            <th className="px-5 py-4 text-left text-xs font-semibold text-slate-500">
                              ...
                              >
                                Price{" "}
                                <SortIcon
                                  active={sortConfig.column === "price"}
                                  direction={sortConfig.direction}
                                />
                              </button>
                            </th>
                              ...
                              >
                                <td className="px-5 py-4">
                                  <input
                                    ...
                                    title={
                                      isConfigured(p)
                                        ? "Uncheck to remove from Item Group"
                                        : "Check to configure Item Group"
                                    }
                                  />
                                </td>
                                <td className="px-5 py-4">
      ...
      {configProduct && (
        <ConfigureModal
          ...
          currentUserId={alertUserId || currentUserId}
